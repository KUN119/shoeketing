<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>Insert title here</title>
</head>
<body>
 <div class="col-ms-7 col-lg-8" style="margin-top: 0px;">
            <h3 style="margin-left: 30px; color: black; font-weight: bolder;">입점 가입 승인</h3>

              <table class="table" style="width: 100%; margin-left: 30px;">
                
                <tbody class="table-group-divider">
                    
                  <tr>
                    <th scope="rowspan-3" style="width: 25%; text-align: center;">
                        <h5 style="margin-top: 18%;">나이키 서울</h5>
                    </th>
                    <td style="text-align: left;margin-bottom: 1; width: 200px;">
                        <p style="font-weight: bolder;">전화번호</p>
                        <p style="font-weight: bolder;">주소</p>
                        <p style="font-weight: bolder;">영업시간</p>
                    </td>
                    <td style="text-align: left;">
                        <p style="font-size: medium;">02-3789-9534</p>
                        <p style="font-size: medium;">서울 서울특별시 중구 명동길 14</p>
                        <p style="font-size: medium;">10:00 ~ 21:30</p>
                    </td>

                    <td style="font-weight: bolder; text-align: center; width: 300px;;">
                            <button type="button" style="margin-top: 15%;" class="btn btn-primary">승인</button>
                            <button type="button" style="margin-top: 15%;" class="btn btn-secondary">거부</button>
                    </td>
                  </tr>

                  <tr>
                    <th scope="rowspan-3" style="width: 25%; text-align: center;">
                        <h5 style="margin-top: 18%;">나이키 서울</h5>
                    </th>
                    <td style="text-align: left;margin-bottom: 1; width: 200px;">
                        <p style="font-weight: bolder;">전화번호</p>
                        <p style="font-weight: bolder;">주소</p>
                        <p style="font-weight: bolder;">영업시간</p>
                    </td>
                    <td style="text-align: left;">
                        <p style="font-size: medium;">02-3789-9534</p>
                        <p style="font-size: medium;">서울 서울특별시 중구 명동길 14</p>
                        <p style="font-size: medium;">10:00 ~ 21:30</p>
                    </td>

                    <td style="font-weight: bolder; text-align: center; width: 300px;">
                            <button type="button" style="margin-top: 15%;" class="btn btn-primary">승인</button>
                            <button type="button" style="margin-top: 15%;" class="btn btn-secondary">거부</button>
                    </td>
                  </tr>

                  <tr>
                    <th scope="rowspan-3" style="width: 25%; text-align: center;">
                        <h5 style="margin-top: 18%;">나이키 서울</h5>
                    </th>
                    <td style="text-align: left;margin-bottom: 1; width: 200px;">
                        <p style="font-weight: bolder;">전화번호</p>
                        <p style="font-weight: bolder;">주소</p>
                        <p style="font-weight: bolder;">영업시간</p>
                    </td>
                    <td style="text-align: left;">
                        <p style="font-size: medium;">02-3789-9534</p>
                        <p style="font-size: medium;">서울 서울특별시 중구 명동길 14</p>
                        <p style="font-size: medium;">10:00 ~ 21:30</p>
                    </td>
                    <td style="font-weight: bolder; text-align: center; width: 300px;">
                        <button type="button" style="margin-top: 15%;" class="btn btn-primary">승인</button>
                        <button type="button" style="margin-top: 15%;" class="btn btn-secondary">거부</button>
                     </td>
                  </tr>

                  <tr>
                    <th scope="rowspan-3" style="width: 25%; text-align: center;">
                        <h5 style="margin-top: 18%;">나이키 서울</h5>
                    </th>
                    <td style="text-align: left;margin-bottom: 1; width: 200px;">
                        <p style="font-weight: bolder;">전화번호</p>
                        <p style="font-weight: bolder;">주소</p>
                        <p style="font-weight: bolder;">영업시간</p>
                    </td>
                    <td style="text-align: left;">
                        <p style="font-size: medium;">02-3789-9534</p>
                        <p style="font-size: medium;">서울 서울특별시 중구 명동길 14</p>
                        <p style="font-size: medium;">10:00 ~ 21:30</p>
                    </td>

                    <td style="font-weight: bolder; text-align: center; width: 300px;">
                        <button type="button" style="margin-top: 15%;" class="btn btn-primary">승인</button>
                        <button type="button" style="margin-top: 15%;" class="btn btn-secondary">거부</button>
                    </td>
                   </tr>
                </tbody>
              </table>

              <nav aria-label="Page navigation example" style="margin-left: 50%;">
                <ul class="pagination">
                  <li class="page-item">
                    <a class="page-link" href="#" aria-label="Previous">
                      <span aria-hidden="true">&laquo;</span>
                    </a>
                  </li>
                  <li class="page-item"><a class="page-link" href="#">1</a></li>
                  <li class="page-item"><a class="page-link" href="#">2</a></li>
                  <li class="page-item"><a class="page-link" href="#">3</a></li>
                  <li class="page-item">
                    <a class="page-link" href="#" aria-label="Next">
                      <span aria-hidden="true">&raquo;</span>
                    </a>
                  </li>
                </ul>
              </nav>
            </div>
</body>
</html>