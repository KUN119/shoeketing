		...
		<form id="shopInfoForm">
            ...
            <h3 style="margin-left: 30px; color: black; font-weight: bolder;">입점 매장 목록</h3>
              <table class="table" style="width: 100%; margin-left: 30px;">
                <tbody id="shopListTbody" class="table-group-divider">
                  <c:choose>
...
$(document).ready(function() {
		...
		$.ajax({
			...
			success : function(data){
				 if(data.deleteResult == "pass"){
					...
					// 삭제 후, 리스트에서 해당 매장이 사라지면 지도도 사라지도록 구현
					$("#map2").empty();
	...
	$("a[name='shopLocationInfo']").on("click", function(e){  // 입점 매장 위치 조회
		e.preventDefault();
...
function fn_selectShopList(pageNo){
	...
	comAjax.setUrl("/sk/brandPage/shopList/paging");
...
function fn_selectShopListCallback(data){ // 페이지 선택 시 화면에 보여줄 결과 콜백함수
	...
	var body = $("#shopListTbody"); // 페이징 한 결과가 반영될 태그
	...
	if(total == 0){ // 결과가 없을 경우
		...
		$.each(data.shopList, function(key, value){ // 가져온 게시글 수 만큼 반복 실행
			str += "<tr id=deleteBtnTr_" + value.SHOP_NUM + ">" + 
			 	   "<th scope='rowspan-3' style='width: 25%; text-align: center;'>" +
			 	   ...
			 	   "<button type='button' class='btn btn-secondary btn-md' name='deleteShop' data-shopNum='" + value.SHOP_NUM + 
			 	   "'>삭제</button><br></td></tr>"
		});
		body.append(str);
