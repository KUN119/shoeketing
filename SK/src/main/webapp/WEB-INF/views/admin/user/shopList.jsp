      ...
      <table class="table" style="text-align: center">
        <thead>
          <tr>
            ...
            <th class="col-3">브랜드명</th>
            <th class="col-4">지점명</th>
            <th class="col-1">탈퇴여부</th>
          </tr>
        </thead>
        <tbody id="shopListBody" class="table-group-divider">
        	<!-- 회원 리스트 -->
        </tbody>
      </table>
...
$(document).ready(function() {
	...
	fn_selectShopList(${page}, '${searchType}', '${keyword}');

  $("#shopListBody").on("click", "a[name='title']", function(e) {  // 공지사항 상세보기
		e.preventDefault();
		const num = $(this).attr("data-num");  //a태그 name이 title 부분 속성의 data-num값 가져와서 변수 num에 저장
		var page = $('#PAGE_INDEX').val();
		var keyword = $('#keyword').val();
		var searchType = $('#searchType').val();
		location.href="/sk/admin/shopDetail?SHOP_NUM=" + num + "&page=" + page + "&keyword=" + keyword + "&searchType=" + searchType;
	});
});
  
  	/* 검색버튼 이벤트에 기존 ajax 함수 제거하고 페이징 함수 연결하기 */
	$("button[name='shopSearch']").on("click", function(e){  //공지사항 검색
		e.preventDefault();
		...
		fn_selectShopList(1, searchType, keyword);
	});

	// 페이징 함수
	function fn_selectShopList(pageNo, searchType, keyword){ /* 매개변수 순서에 맞게 추가하기!! */
		var comAjax = new ComAjax();
		comAjax.setUrl("/sk/admin/shopList/paging");
	...
	function fn_selectShopListCallback(data){ // 페이지 선택 시 화면에 보여줄 결과 콜백함수
		...
		var body = $("#shopListBody"); // 페이징 한 결과가 반영될 태그
		...
		if(total == 0){ // 결과가 없을 경우
			...
			var str = "";
			$.each(data.shopList, function(key, value){ // 가져온 게시글 수 만큼 반복 실행
				str += "<tr>" + 
							"<td>" + value.SHOP_NUM + "</td>" + 
							"<td><a href='#' name='title' data-num='" + value.SHOP_NUM + "'>" + value.SHOP_ID + "</a></td>" +
							"<td>" + value.SHOP_BRAND + "</td>" + 
							"<td>" + value.SHOP_NAME + "</td>" + 
							"<td>" + value.SHOP_DEL_GB + "</td>" + 
						"</tr>";
			});
			body.append(str);
		}
