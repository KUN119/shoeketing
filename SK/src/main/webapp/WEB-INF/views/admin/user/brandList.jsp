...
<body>
    <div class="col-10" style="margin-top: 0px">
      ...
      <table class="table" style="text-align: center">
        ...
        <tbody id="brandListBody" class="table-group-divider">
          <!-- 회원 리스트 -->
        </tbody>
      </table>
...
$(document).ready(function() {
	...
	fn_selectBrandList(${page}, '${searchType}', '${keyword}');

  $("#brandListBody").on("click", "a[name='title']", function(e) {  // 공지사항 상세보기
		e.preventDefault();
		const num = $(this).attr("data-num");  //a태그 name이 title 부분 속성의 data-num값 가져와서 변수 num에 저장
		var page = $('#PAGE_INDEX').val();
		var keyword = $('#keyword').val();
		var searchType = $('#searchType').val();
		location.href="/sk/admin/brandDetail?BRAND_NUM=" + num + "&page=" + page + "&keyword=" + keyword + "&searchType=" + searchType;
	});
});
  
  	/* 검색버튼 이벤트에 기존 ajax 함수 제거하고 페이징 함수 연결하기 */
	$("button[name='brandSearch']").on("click", function(e){  //공지사항 검색
		e.preventDefault();
		...
		fn_selectBrandList(1, searchType, keyword);
	});

	// 페이징 함수
	function fn_selectBrandList(pageNo, searchType, keyword){ /* 매개변수 순서에 맞게 추가하기!! */
		var comAjax = new ComAjax();
		comAjax.setUrl("/sk/admin/brandList/paging");
		comAjax.setCallback("fn_selectBrandListCallback");
	...
	// 페이징 콜백 함수
	function fn_selectBrandListCallback(data){ // 페이지 선택 시 화면에 보여줄 결과 콜백함수
		var total = data.TOTAL;
		var body = $("#brandListBody"); // 페이징 한 결과가 반영될 태그
		...
		if(total == 0){ // 결과가 없을 경우
			...
			var params = {
				...
				totalCount : total,
				recordCount : 10,
				eventName : "fn_selectBrandList" // 페이징 함수이름 동일하게
			};
			gfn_renderPaging(params);
			
			var str = "";
			$.each(data.brandList, function(key, value){ // 가져온 게시글 수 만큼 반복 실행
				str += "<tr>" + 
							"<td>" + value.BRAND_NUM + "</td>" + 
							"<td><a href='#' name='title' data-num='" + value.BRAND_NUM + "'>" + value.BRAND_ID + "</a></td>" +
							"<td>" + value.BRAND_NAME + "</td>" + 
							"<td>" + value.BRAND_DEL_GB + "</td>" + 
						"</tr>";
			});
			body.append(str);
		}
